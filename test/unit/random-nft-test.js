...
!developmentChains.includes(network.name)
    ? describe.skip
    : describe("Random Nft Test", function () {
          ...
          describe("Constructor", () => {
              it("Should Initialize the NFT Correctly", async () => {
                  const dogTokenUris = await randomIpfsNft.getDogTokenUri(0)
                  assert(dogTokenUris.includes("ipfs://"))
              })
          })
          describe("requestNft", () => {
              it("Should revert if No value is send with the request", async () => {
                  await expect(randomIpfsNft.requestNft()).to.be.revertedWith(
                      "RandomIpfsNft__NeedMoreETHSent"
                  )
              })
              it("Should revert with if the sent value is Less than the mint Fee", async () => {
              ...
              it("Should request an Nft once There is Enough ETH", async () => {
                  const mintFee = await randomIpfsNft.getMintFee()
                  await expect(randomIpfsNft.requestNft({ value: mintFee.toString() })).emit(
                      randomIpfsNft,
                      "NftRequested"
                  )
              })
